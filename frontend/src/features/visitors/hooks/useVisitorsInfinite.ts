...
export function useVisitorsInfinite(committedSearch: string | null) {
  ...
  const query = useInfiniteQuery({
    queryKey: visitorsKeys.infinite(isSearching ? committedSearch!.trim() : null),
    initialPageParam: 0 as number,
    queryFn: async ({ pageParam }) => {
      if (isSearching) {
        const list = await searchVisitors({ q: committedSearch!.trim() });
        const page: ModePage = { visitors: list, next_cursor: 0, has_more: false };
        return page;
      }
  ...
  const visitors: Visitor[] = (query.data?.pages ?? []).flatMap((p) => p.visitors);
